...
resource "aws_security_group_rule" "dev_container" {
  type            = "ingress"
  from_port       = 8080
  to_port         = 8080
  protocol        = "tcp"
  ...
  security_group_id = "${aws_security_group.dev_container.id}"
}

resource "aws_security_group_rule" "dev_container_outbound" {
  type             = "egress"
  from_port        = 0
  to_port          = 0
  protocol         = -1
